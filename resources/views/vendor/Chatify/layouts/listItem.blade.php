...
@if ($get == 'users' && !!$lastMessage)
    ...
    // check row exists in table sessions where user_id = $user->id
    $isActivity = \DB::table('sessions')->where('user_id', $user->id)->count();
    ?>
        ...
        <tr data-action="0">
            ...
            <td>
                ...
                <span>
                    ...
                    {{-- Last message body --}}
                    @if ($lastMessage->attachment ?? null == null)
                        {!! $lastMessageBody !!}
                    @else
...
{{-- -------------------- Group List -------------------- --}}
@if ($get == 'group')
    <table class="messenger-group-item" data-contact="{{ $group->id }}">
        <td>
        ...
        {{-- center side --}}
        <td>
            <p data-id="{{ $group->id }}" data-type="user">
