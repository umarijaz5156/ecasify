...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('assets/plugins/daterangepicker-master/daterangepicker.css') }}">
    ...
    <script>
        ...
        channel.bind('case-notification', function(data) {
            ...
            show_toastr('{{ __('Success') }}', data.data.message, 'success')
...
<body class="{{ $color }}">
    ...
    @if (Auth::user()->type != 'super admin')
        {{-- <div class="app-time-tracker" id="draggableTimer" onmousedown="startDrag(event)" onmouseup="stopDrag(event)">

            <div class="timer-img-wrapper"   id="startTimerButton" data-toggle="tooltip"
                title="{{ __('Tap to Start Timer!') }}" data-bs-original-title="{{ __('Tap to Start Timer!') }}"
        ...
        <div class="app-time-tracker" id="draggableTimer" onmousedown="startDrag(event, 'draggableTimer')"
            ...
            <div class="timer-img-wrapper" id="startTimerButton" onmousedown="startDrag(event, 'startTimerButton')"
                data-toggle="tooltip" title="{{ __('Tap to Start Timer!') }}"
                data-bs-original-title="{{ __('Tap to Start Timer!') }}" data-bs-placement="top"
            ...
            <div class="timerdiv" id="timerDiv" style="display: none;">
                <div class="text-end">
                    <button type="button" class="btn-close" data-toggle="tooltip" data-bs-placement="top"
                        data-bs-toggle="tooltip" title="{{ __('Minimize') }}"
                        data-bs-original-title="{{ __('Minimize') }}" id="clossBtnTimer"></button>
                </div>
                ...
                <div class="counter-started">
                    {{-- <h5>Logged in today : <span class="counter-started-time">2h45m</span></h5> --}}
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="timerFormId">
                        ...
                        <label for="witness_name">{{ __('Notes') }}</label>
    ...
    <style>
        .toast-success-icon:before {
            content: url("{{ asset('assets/images/notification/ok-48.png') }}");
        }

        .toast-error-icon:before {
            content: url("{{ asset('assets/images/notification/high_priority-48.png') }}");
        }
        ...
        <script>
            ...
            function updateUI(status) {
                ...
                if (status === 'started') {
                    ...
                    // startTimerButton.setAttribute('title', '{{ __('Timer is Minimized and Active') }}');
                    document.getElementById('statusElement').innerHTML =
                        `
            <img style="cursor: pointer;" width='45' height='45' id="timerimgid" onmousedown="startDrag(event, 'timerimgid')" class="img-fluid" src="{{ asset('storage/uploads/stopwatch.gif') }}" alt="Stopwatch">`;
                    startTimerButton.setAttribute('data-bs-original-title', '{{ __('Timer is Minimized and Active') }}');
            ...
            `;
                    // startTimerButton.setAttribute('title', '{{ __('Tap to Start Timer!') }}');
                    startTimerButton.setAttribute('data-bs-original-title', '{{ __('Tap to Start Timer!') }}');
    ...
    `;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            show_toastr('{{ __('Success') }}', 'Timesheet successfully created', 'success')
                        } else {
                            $('#timerModal').modal('hide');
                            show_toastr('{{ __('Error') }}', 'Error creating timesheet', 'error')
                        }
                        // rest timerFormId 
            ...
            function resetTimer(model) {
                ...
                if (model != 'model') {
                    show_toastr('{{ __('Success') }}', 'Time Discard successfully', 'success')
                }
            ...
            const WEBSITE_KEY = '{!! env('APP_URL') !!}'; // Root URL of the website
            ...
            function checkInactivity() {
                ...
                if (elapsedTime >= ACTIVITY_TIMEOUT && isLastTab && timerData.status == 'started') {
                    ...
                    updateTimerDisplay();
                    // show alert
                    show_toastr('{{ __('Update') }}', 'Timer Paused due to Inactivity',
                        'error')
                }
            }
            ...
            $(document).ready(function() {
                ...
                modal.on('hidden.bs.modal', function() {
                    if (closedByClickOutside) {
                        show_toastr('{{ __('Update') }}', 'Timer has been paused.', 'error');
