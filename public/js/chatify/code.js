...
function disableOnLoad(disable = true) {
 if (disable) {
   ...
   // show star button
   if (getMessengerId() != auth_id && chat_type != 'group') {
...
*-------------------------------------------------------------
*/
function errorMessageCard(id,errorMasg = '')
{
  messagesContainer
...
*-------------------------------------------------------------
*/
function IDinfo(id,type = 'chat') {
 var removeGroupContacts = true;
 ...
 if (messenger != 0) {
   ...
   if(type == 'groups'){
    //  set chat_type gloal variable = group
     chat_type = 'group';
    groupContacts(id,type);
    removeGroupContacts = false;
  }else{
    //  set chat_type gloal variable = chat
     chat_type = 'chat';
  }
   // Get info
   $.ajax({
     url: url + "/idInfo",
     method: "POST",
     data: { _token: csrfToken, id,type:type },
     dataType: "JSON",
     success: (data) => {
       if (!data?.fetch) {
         NProgress.done();
       ...
       if(removeGroupContacts){
      $(".group-contact-list").empty();
      $(".group-contact-title").empty();
       }
       // avatar photo
       ...
       const uri = url[4];
       const type = uri == 'group' ? 'groups' : 'chat';
       fetchMessages(id, true,type);
...
function sendMessage() {
 ...
 if (inputValue.length > 0 || hasFile) {
   ...
   // get url uri 3
    const url = window.location.href.split("/");
    const uri = url[4];
    const type = uri == 'group' ? 'group' : 'chat';
   ...
   $.ajax({
     ...
     success: (data) => {
       if (data.error > 0) {
         ...
         errorMessageCard(tempID,data.error_msg);
         console.log(tempID);
...
function fetchMessages(id, newFetch = false,type='chat') {
 if (newFetch) {
   messagesPage = 1;
...
var extractedInt = getMessengerId().match(/\d+/)?.[0] || "No integer found";
var channelGroup = pusher.subscribe(`${channelName}.${extractedInt}`);
...
// bindGroupChat()
  ...
  function bindGroupChat(){
          // groupChat
      channelGroup.bind("groupChat", function (data) {
        var caseId = parseInt(getMessengerId().match(/\d+/)[0]);
        console.log(getMessengerId(),'caseid => ',caseId,auth_id,'endl',data.from_id);
        if ( data.case_id == getMessengerId() && data.from_id != auth_id) {
          $(".messages").find(".message-hint").remove();
          var $message = $(data.message);

          var username = authUser.type != 'client' ? data.from_id_name : data.company_name;
          var from_username = data.from_id == authUser.id ? 'You' : username;
          // Find and modify elements with class "sender-name" and "is_team"
        
          $message.find(".sender-name").text(from_username);
          if(authUser.type == 'client'){
            $message.find(".is_team").removeClass("is_team");
          }
          messagesContainer.find(".messages").append($message.prop('outerHTML'));
          scrollToBottom(messagesContainer);
          makeSeen(true);
          // remove unseen counter for the user from the contacts list
          $(".messenger-list-item[data-contact=" + getMessengerId() + "]")
...
function getGroupList() {
  $(".messenger-groups").html(avatarLoading(4));
  $.ajax({
    url: url + "/getContacts",
    method: "GET",
    data: { _token: csrfToken,type:'groups' },
    dataType: "JSON",
    success: (data) => {
      if (data.count > 0) {
      console.log('werwer',data);
        ...
        $(".groups-section").show();
        $(".messenger-groups").html(data.groups);
      } else {
        $(".groups-section").hide();
      }
      // update data-action required with [responsive design]
...
*-------------------------------------------------------------
*/
function groupContacts(case_id,type = 'chat') {
  $.ajax({
    url: url + "/idInfo",
    method: "POST",
    data: { _token: csrfToken, case_id: case_id, type:type ,callFunction:'groupContacts' },
    dataType: "JSON",
    success: (data) => {
      $(".group-contact-list").html(data.contacts);
...
*-------------------------------------------------------------
*/
function getSharedPhotos(user_id,type = 'chat') {
 $.ajax({
   url: url + "/shared",
   method: "POST",
   data: { _token: csrfToken, user_id: user_id, type:type },
   dataType: "JSON",
   success: (data) => {
...
$(document).ready(function () {
  ...
  // // Toggle sendAndTime on click of .message
  // $(document).on("click", ".message", function() {
  //   // Find the closest .message-card-content element and then toggle its .sendAndTime
  //   $(this).closest(".message-card-content").find(".sendAndTime").toggle();
  // });
 // get contacts list
 getContacts();
...
getGroupList();
 ...
 const subscribedChannel = {};
  ...
  // set item active on click
  $("body").on("click", ".messenger-group-item", function () {
    $(".messenger-group-item").removeClass("m-list-active bg-primary");
    $(this).addClass("m-list-active bg-primary");
    const uid = $(this).attr("data-contact");

  routerPush(document.title, `${url}/group/${uid}`);
   // Check if the channel is already subscribed
   channelGroup.unsubscribe();
  setMessengerId(uid);
    IDinfo(uid,'groups');
    updateSelectedContact(uid,'groups');
    channelGroup = pusher.subscribe(`${channelName}.${uid}`);
    if (!subscribedChannel[uid]) {

    bindGroupChat();
// Mark the channel as subscribed
subscribedChannel[uid] = true;
}else {
    // If already subscribed, you can optionally handle this case
    // For example, you can display a message or take some other action
    console.log(`Channel ${channelName}.${uid} is already subscribed.`);
  }
});
 ...
 // make grops card drafable on click slide
 hScroller(".messenger-groups");
 ...
 // click action for group button
  $("body").on("click", ".group-list-item", function () {
    ...
    channelGroup = pusher.subscribe(`${channelName}.${uid}`);
    if (!subscribedChannels[uid]) {

    bindGroupChat();
// Mark the channel as subscribed
subscribedChannels[uid] = true;
}else {
    // If already subscribed, you can optionally handle this case
    // For example, you can display a message or take some other action
    console.log(`Channel ${channelName}.${uid} is already subscribed.`);
...
//  });
 ...
 actionOnScroll(
   ...
   function () {
    ...
    //   const uri = url[4];
    //   const type = uri == 'group' ? 'groups' : 'chat';
    //  fetchMessages(getMessengerId(),true,type);
   },
   true
 );
 //Contacts pagination
